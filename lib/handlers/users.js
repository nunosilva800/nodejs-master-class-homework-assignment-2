...
resource.get = (data, cb) => {
  ...
  let email = null;
  if (typeof(data.queryStringObject.email) == 'string' &&
      data.queryStringObject.email.length > 0) {
  ...
  auth.verifyToken(token, email, (valid) => {
    if (valid) {
      // lookup user
      _data.read('users', email, (err, data) => {
        if (!err && data) {
...
resource.put = (data, cb) => {
  ...
  if (!(name || address || password)) {
    return cb(400, { 'Error' : 'Missing optional fields' });
  }
