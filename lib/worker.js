...
const mailgunHelpers = require('./mailgun_helpers');
        ...
        if (!err) {
          _data.delete('tasks', taskData.id, (err) => {
            if (!err) {
              ...
              debug("Error deleting completed task: " + err, taskData);
            }
          });
        } else {
          debug("Error performing task: " + err, taskData);
        }
      }
    );
  } else {
...
worker.jobs = {
  'OrderReceiptEmail' : (args, cb) => {
    debug('Performing OrderReceiptEmail with ', args);
    const orderId = args.orderId;
    _data.read('orders', orderId, (err, orderData) => {
      if (!err && orderData) {
        mailgunHelpers.sendOrderReceipt(orderData, (err) => {
          if (!err) {
            cb(false);
          } else {
            cb('Error sending order receipt email: ' + err);
          }
        });
      } else {
        cb('Order not found');
